...
const parseIntOrEmpty = (raw: string): NumOrEmpty => {
  const s = raw.trim();
  if (s === "") return "";
...
const parseFloatOrEmpty = (raw: string): NumOrEmpty => {
  const s = raw.trim();
  if (s === "") return "";
  const v = Number(s.replace(",", "."));
  if (!Number.isFinite(v) || v < 0) return "";
...
export default function RecyclerAddRecycling() {
  ...
  const onInt = (key: string, val: string) => {
    const parsed = parseIntOrEmpty(val);
    setBats(p => ({ ...p, [key]: { ...p[key], pcs: parsed } }));
  };

  const onFloat = (key: string, field: "weight_kg" | "price_ron", val: string) => {
    const parsed = parseFloatOrEmpty(val);
    setBats(p => ({ ...p, [key]: { ...p[key], [field]: parsed } }));
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!canSubmit) {
      setErr("Completează cel puțin un câmp (> 0) pe oricare linie.");
      return;
    }
    ...
    try {
      ...
      for (const k of ALL_KEYS) {
        ...
        const ron = toNum(ln.price_ron);
        if (pcs > 0 || kg > 0 || ron > 0) {
          const line: { pcs?: number; weight_kg?: number; price_ron?: number } = {};
          if (pcs > 0) line.pcs = pcs;
          if (kg > 0) line.weight_kg = round2(kg);
          if (ron > 0) line.price_ron = round2(ron);
          batteries[k] = line;
        }
      }

      await api.createRecycling({ batteries });
      setOk("Reciclarea a fost creată.");
      ...
      // nav("/recycler/recyclings");
    } catch (e: any) {
      setErr(e?.message || "Eroare la creare reciclare");
    } finally {
      setSaving(false);
    }
  };
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
                ...
                <tr key={k}>
                  ...
                  <td style={{ textAlign: "center" }}>
                    <input
                      ...
                      onChange={e => onInt(k, e.target.value)}
                      style={{ width: 120 }}
                    />
                  </td>
                  <td style={{ textAlign: "center" }}>
                    <input
                      ...
                      onChange={e => onFloat(k, "weight_kg", e.target.value)}
                      style={{ width: 120 }}
                    />
                  </td>
                  <td style={{ textAlign: "center" }}>
                    <input
                      ...
                      onChange={e => onFloat(k, "price_ron", e.target.value)}
                      style={{ width: 140 }}
                    />
                  </td>
                </tr>
              ...
              <tr style={{ background: "#fafafa", fontWeight: 600 }}>
                ...
                <td style={{ textAlign: "center" }}>{fmt(totals.totalWeight)} kg</td>
              </tr>
              <tr style={{ background: "#fafafa", fontWeight: 600 }}>
                <td colSpan={2} />
                <td style={{ textAlign: "right", paddingRight: 8 }}>Total valoare</td>
                <td style={{ textAlign: "center" }}>{fmt(totals.totalCost)} lei</td>
              </tr>
            </tbody>
          </table>
        </section>
