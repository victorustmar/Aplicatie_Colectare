...
// always return a number (NaN if empty/invalid) to avoid 'number | ""'
...
export default function ProducerAddPackage() {
  const { user } = useAuth();
  ...
  const onInt = (key: string, val: string) => {
    const n: number = parseNum(val);
    setBats(p => ({
      ...p,
      [key]: {
        ...p[key],
        pcs: !Number.isFinite(n) || n < 0 ? '' : Math.floor(n),
      },
    }));
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!user?.company_id) { setErr('Compania utilizator lipsă.'); return; }
    ...
    try {
      ...
      await api.createPackage({
        producer_company_id: user.company_id!,
        batteries,
      });
      setOk('Pachetul a fost creat.');
      const reset: BatteriesState = {};
      for (const k of ALL_KEYS) reset[k] = { pcs: '', weight_kg: '', price_ron: '' };
      setBats(reset);
    } catch (e: any) {
      setErr(e?.message || 'Eroare la creare pachet');
    } finally {
      setSaving(false);
    }
  };
